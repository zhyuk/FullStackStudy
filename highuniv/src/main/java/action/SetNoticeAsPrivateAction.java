...
public class SetNoticeAsPrivateAction implements Action {
    ...
    public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
         ...
         if (!"admin".equals(userId)) {
             ...
             response.getWriter().println("alert('공지 취소는 관리자만 가능합니다.');");
        ...
        if (noticeIds != null && noticeIds.length > 0) {
            ...
            boolean isUpdateSuccess = noticeUpdateService.setNoticesAsPrivate(noticeIds);
            ...
            if (isUpdateSuccess) {
                ...
                response.setContentType("text/html;charset=UTF-8");
                response.getWriter().println("<script>");
                response.getWriter().println("alert('공지 취소 실패');");
                response.getWriter().println("history.back();");
                response.getWriter().println("</script>");
            }
        } else {
