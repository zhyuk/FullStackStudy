		...
		<head>
			...
			<style>
				...
				td.mShow div ul li {
					...
					justifit-content: center;
					align-items: center;
				}
				
				td.mShow div input[type=text] {
					width: 100%;
					height: 50%;
				}
		...
		<body class="pretendard">
			...
			<table>
				<tr>
					...
					<th>학번</th>
					<th>이름</th>
					<th class="mHide">상태</th>
					<th class="mHide">비고</th>
				</tr>
				...
				<c:forEach var="attend" items="${AttendList}">
					<tr>
						...
						<td class="mHide"><span class="show status-msg">${attend.status}</span>
							<div class="radio-Box hide">
								<ul>
									<li><label for="attend${num}">출석</label> <input type="radio" name="attend${num}"
											id="attend${num}" value="출석" <c:set var="i" value="출석" />
										<c:if test="${i eq attend.status}">checked</c:if>>
									</li>
									<li><label for="late${num}">지각</label> <input type="radio" name="attend${num}"
											id="late${num}" value="지각" <c:set var="i2" value="지각" />
										<c:if test="${i2 eq attend.status}">checked</c:if>>
									</li>
									<li><label for="ealry${num}">조퇴</label> <input type="radio" name="attend${num}"
											id="ealry${num}" value="조퇴" <c:set var="i3" value="조퇴" />
										<c:if test="${i3 eq attend.status}">checked</c:if>>
									</li>
									<li><label for="absence${num}">결석</label> <input type="radio" name="attend${num}"
											id="absence${num}" value="결석" <c:set var="i4" value="결석" />
										<c:if test="${i4 eq attend.status}">checked</c:if>>
									</li>
								</ul>
							</div>
						</td>
						<td class="mHide"><span class="show">${attend.attend_remarks}</span> <input type="text" id="comment${num}"
								class="hide" value="${attend.attend_remarks}"></td>
					</tr>
					
					<tr>
						<td class="mShow" colspan="3">
							<div class="radio-Box hide">
								<ul>
									<li><label for="attend${num}">출석</label> <input type="radio" name="attend${num}"
											id="attend${num}" value="출석" <c:set var="i" value="출석" />
							...
							<div><span class="show">${attend.attend_remarks}</span> <input type="text" id="comment${num}" class="hide" value="${attend.attend_remarks}"></div>
						</td>
					</tr>
									...
									$("input.chk:checked")
										.each(
											function () {
												...
												let status = $row.find("input[name^='attend']:checked").val(); // 출석 상태
												let comment = $row.find("input[type=text]").val(); // 비고
												let studentId = $row.find("td.student_id").text();
												let subjectName = $row
													.find(
														"td.subject_name")
													.text();
