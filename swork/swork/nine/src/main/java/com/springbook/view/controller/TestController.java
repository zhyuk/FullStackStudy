...
public class TestController {
	...
	/*
	 * @RequestBody 이 어노테이션이 붙은 파라미터에는 http요청의 본문(body)이 그대로 전달된다. 일반적인 GET/POST의 요청
	 * 파라미터라면 @RequestBody를 사용할 일이 없다. 반면에 xml이나 json기반의 메시지를 사용하는 요청의 경우에 이 방법이 매우
	 * 유용하다. HTTP 요청의 바디내용을 통째로 자바객체로 변환해서 매핑된 메소드 파라미터로 전달해준다.
	 * 
	 * @ResponseBody 자바객체를 HTTP요청의 바디내용으로 매핑하여 클라이언트로 전송한다. jackson-databind 라이브러리가
	 * 있는 경우 @ResponseBody를 사용하여 데이터를 전송하면 데이터의 타입을 확인하여 자동으로 JsonArray이나 JsonObject
	 * 자료형으로 변환하여 반환해준다.
	 * 
	 * 참고사이트 : https://cheershennah.tistory.com/179
	 *
	 *  @RequestBody UserVO getUserVO : 
	 *  커맨드 객체에서 json 형태의 데이터를 자동으로 setter 주입할 수 없음.
	 *  @RequestBody 어노테이션을 통해 데이터를 파라미터 형태로 파싱하여 자동 setter 처리할 수 있게됨.
	 */
...
//		String formatedNow = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
	...
	@ResponseBody
	// BoardVO vo : 커맨드 객체가 자동으로 setter 주입이 가능한 데이터는 form-data(파라미터)만 가능하다. json 형태의 데이터를 setter처리 불가.
...
//		return "WEB-INF/board/getBoardList.jsp"; // @ResponseBody가 없는 경우 해당 경로에 있는 해당 파일을 html로 컴파일한 값을 반환
